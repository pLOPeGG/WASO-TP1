...
public class ServiceObjetMetier {
    ...
    public void getPersByID(Integer idPersonne)throws ServiceException{
        try {
            List<Object[]> listePersonne = this.dBConnection.launchQuery("SELECT PersonneID, Nom, Prenom, Mail FROM PERSONNE WHERE PersonneID = ? ORDER BY PersonneID", idPersonne);

            if(listePersonne.size() != 1){
                throw new ServiceException("Erreur DB-Personne : PK");
            }
            ...
            throw new ServiceException("Exception in SOM getPersonneByID", ex);
        }
    }
