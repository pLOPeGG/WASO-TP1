    ...
    public void rechercherClientParDenomination(String denomination, String ville) throws ServiceException{
        ...
        try {
            ...
            for(JsonElement clientElement : jsonOutputClientListe){
                ...
                for(JsonElement personneIDElement : personnesID){
                    String personneIDString = personneIDElement.getAsString();
                    
                    JsonObject personneContainer = this.jsonHttpClient.post(this.somPersonneUrl, new BasicNameValuePair("SOM", "getPersByID"), new BasicNameValuePair("id-personne", personneIDString));//TODO : pour Vincent : "idP"
                    /* pour bosser avec Vincent
                    System.out.println(personneContainer+ "********************");
                    for(JsonElement pers : personneContainer.get("personnes").getAsJsonArray()){
                        outputPersonnes.add(pers.getAsJsonObject());
                    }
*/
                    outputPersonnes.add(personneContainer.get("personne")); // personnes avec vincent
                }
